...
### B-4: Routines（Anthropic管理クラウド）

Anthropicのクラウド上でスケジュール / API / GitHubイベントで起動。Claude Code on the web の自動化レイヤー。

| 項目 | 内容 |
|------|------|
| PC依存 | なし |
| 認証 | **Pro/Maxサブスク内**（公式に許諾された使い方） |
| 追加コスト | サブ込み（**日次上限あり**：Pro 5/日, Max 15/日, Team/Ent 25/日） |
| セキュリティ | △ 作成者のフル権限を継承、実行中の承認プロンプトなし → プロンプトインジェクションが直ヒット |
| 強み | サブスク内・PC不要・GitHub連携・OS保守ゼロ |
| 弱点 | Research Preview段階、`claude/` プレフィックスへのpush限定（初期）、本番SSHデプロイ実例ほぼ無、日次上限 |
| 向く用途 | デプロイ後のスモークテスト・週次ドキュメント検査・PRトリアージ等の補助層 |
...
| B-2: VPS常駐 | なし | Pro/Max or API | $5〜30 | △ | 高 |
| B-3: cron + API直叩き | なし | API key | $1未満〜 | ◎ | 低 |
| **B-4: Routines** | なし | **Pro/Maxサブ** | **サブ込（日次上限）** | △ | 高 |

### B-4 のポジション

- **A の "PC不要版"** に近い：サブスク内で完結、PC非依存、Anthropic公式
- ただし**承認プロンプトなしのフル権限実行**は本番SSHデプロイには重い
- 公式・コミュニティとも「デプロイ後の verification 層」推奨。**デプロイ本体はB-1かB-2に任せ、Routinesはその後段**が定型
